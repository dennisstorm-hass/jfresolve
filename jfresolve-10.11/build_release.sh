...
echo ""
echo "To update repository.json checksum automatically (from repo root):"
echo "  CHECKSUM=\$(md5 -q jfresolve-10.11/jfresolve.zip 2>/dev/null || md5sum jfresolve-10.11/jfresolve.zip | awk '{print \$1}')"
echo "  Then replace the placeholder checksum for version ${VERSION} with \$CHECKSUM"
