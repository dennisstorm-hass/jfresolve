...
/// <summary>
/// Resolves TorBox/Torrentio stream URLs to stable requestdl permalinks (redirect=true).
/// TorBox CDN URLs are short-lived; each playback/range request must re-resolve through the permalink.
/// </summary>
public class TorBoxStreamService
{
    private const string TorBoxApiBase = "https://api.torbox.app/v1/api/torrents";
    ...
    /// <summary>
    /// Returns a TorBox requestdl permalink when the URL is a Torrentio TorBox resolve link and we can resolve torrent/file ids.
    /// </summary>
    public async Task<string?> TryNormalizeToRequestDlPermalinkAsync(
        string streamUrl,
        string? torBoxApiKey,
        CancellationToken cancellationToken = default)
    {
        ...
        if (IsTorBoxRequestDlPermalink(streamUrl))
            return EnsureRedirectTrue(streamUrl);

        if (!TryParseTorrentioTorBoxUrl(streamUrl, out var infoHash, out var fileIndex))
            return null;

        var fromApi = await TryBuildPermalinkFromMyListAsync(torBoxApiKey, infoHash, fileIndex, cancellationToken);
        ...
        _logger.LogDebug(
            "Jfresolve: Could not resolve TorBox requestdl permalink for {Url}, using Torrentio resolve URL",
            streamUrl);
        return null;
    }
    ...
    private async Task<string?> TryBuildPermalinkFromMyListAsync(
        string apiKey,
        string infoHash,
        int fileIndex,
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            using var request = new HttpRequestMessage(HttpMethod.Get, $"{TorBoxApiBase}/mylist?bypass_cache=true");
            ...
            foreach (var torrent in data.EnumerateArray())
            {
                ...
                return BuildRequestDlPermalink(apiKey, torrentId, fileId);
            }
        }
        catch (Exception ex)
        {
    ...
    private static string ResolveFileId(JsonElement torrent, int fileIndex)
    {
        ...
        // Torrentio file index often matches TorBox file id for single-file torrents
    ...
    {
        return $"{TorBoxApiBase}/requestdl?token={Uri.EscapeDataString(apiKey)}&torrent_id={Uri.EscapeDataString(torrentId)}&file_id={Uri.EscapeDataString(fileId)}&redirect=true";
    }
